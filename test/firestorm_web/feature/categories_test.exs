defmodule FirestormWeb.Feature.CategoriesTest do
  ...
  test "category show shows posts count for threads", %{session: session} do
    ...
    session
    # We have to log in now because we aren't going to track views by anonymous
    # users in this table.
    |> log_in_as(user)
    ...
    # We're adding this bit, to visit a thread and then revisit its parent
    # category. Visiting the thread will mark all of its posts viewed, for now.
    session
    |> log_in_as(user)
